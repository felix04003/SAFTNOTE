import { Api } from '../api';
import { escapeHtml, cn, toast, openModal, closeModal } from '../ui';
import { PAGE_HOOKS } from '../ens-router';

export const PageEnsNotes: any = {
  _data: [],
  _classes: [],      // [{classe_id, classe, affectation_id, matiere}]
  _periodes: [],
  _evalCourant: null,
  _filtreClasseId: '',
  _filtreStatut: '',

  async init() {
    await Promise.all([this._chargerClasses(), this._chargerPeriodes()]);
    this._peuplerFiltreClasses();
    await this.charger();
  },

  async _chargerClasses() {
    try {
      const res = await Api.get('/enseignants/moi/classes');
      this._classes = res.data || [];
    } catch (e: any) { this._classes = []; }
  },

  async _chargerPeriodes() {
    try {
      const r = await Api.get('/annees-scolaires/courante');
      this._periodes = (r.data && r.data.periodes) || [];
    } catch (e: any) { this._periodes = []; }
  },

  _peuplerFiltreClasses() {
    const sel = document.getElementById('ens-fil-classe') as HTMLSelectElement | null;
    if (!sel) return;
    // Dédupliquer les classes (un enseignant peut avoir plusieurs matières dans une même classe)
    const vues: Record<string, boolean> = {};
    const classes = this._classes.filter(function(c: any) {
      if (vues[c.classe_id]) return false;
      vues[c.classe_id] = true;
      return true;
    });
    sel.innerHTML = '<option value=””>Toutes mes classes</option>' +
      classes.map(function(c: any) {
        return '<option value=”' + escapeHtml(String(c.classe_id || '')) + '”>' + escapeHtml(c.classe || '') + '</option>';
      }).join('');
    if (PageEnsNotes._filtreClasseId) sel.value = PageEnsNotes._filtreClasseId;
  },

  // Appelé depuis ens-classes (raccourci) — DOIT être suivi d'une navigation goto('ens-notes')
  // Le filtre est appliqué lors du prochain init() (charger() + _peuplerFiltreClasses())
  filtrerParClasse: function(classeId: string) {
    this._filtreClasseId = classeId;
  },

  filtrerClasse: function(classeId: string) { this._filtreClasseId = classeId; this.charger(); },
  filtrerStatut: function(statut: string) { this._filtreStatut = statut; this.charger(); },

  async charger() {
    const tbody = document.getElementById('tb-ens-eval') as HTMLElement | null;
    if (!tbody) return;
    tbody.innerHTML = '<tr><td colspan=”7” style=”text-align:center;padding:30px;color:var(--g400)”>Chargement…</td></tr>';

    try {
      const params: Record<string, any> = {};
      if (this._filtreClasseId) params['classe_id'] = this._filtreClasseId;
      if (this._filtreStatut)   params['statut']    = this._filtreStatut;

      const res = await Api.get('/evaluations', params);
      this._data = res.data || [];
      this._renderTable(this._data);
    } catch (e: any) {
      tbody.innerHTML = '<tr><td colspan=”7” style=”text-align:center;padding:30px;color:var(--rouge)”>' + escapeHtml(e.message || 'Erreur de chargement') + '</td></tr>';
    }
  },

  _renderTable: function(evals: any[]) {
    const tbody = document.getElementById('tb-ens-eval') as HTMLElement | null;
    if (!tbody) return;

    if (!evals.length) {
      tbody.innerHTML = '<tr><td colspan=”7” style=”text-align:center;color:var(--g400);padding:30px”>Aucune évaluation — créez-en une avec “+ Nouvelle évaluation”</td></tr>';
      return;
    }

    tbody.innerHTML = evals.map(function(ev: any) {
      const moy = ev.moyenne_classe != null ? ev.moyenne_classe : null;
      const st = ev.statut || 'non_saisie';
      const badge = st === 'publiee'
        ? '<span class=”badge bs”>Publiée</span>'
        : st === 'brouillon'
          ? '<span class=”badge bw”>Brouillon</span>'
          : '<span class=”badge bd”>À saisir</span>';

      const evalId = escapeHtml(String(ev.id || ''));

      return '<tr style=”cursor:pointer” onclick=”PageEnsNotes.ouvrirSaisie(\'' + evalId + '\')”>' +
        '<td class=”nc”>' + escapeHtml(ev.matiere || '—') + '</td>' +
        '<td><span class=”badge bp”>' + escapeHtml(ev.classe || '—') + '</span></td>' +
        '<td><span class=”badge bo”>' + escapeHtml(ev.type || '—') + '</span></td>' +
        '<td style=”font-family:\'Space Mono\',monospace;font-size:11.5px”>' + escapeHtml(ev.date_evaluation || '—') + '</td>' +
        '<td>' + (moy != null ? '<span style=”font-weight:700;color:' + cn(moy) + '”>' + moy + '/20</span>' : '<span class=”badge bd”>—</span>') + '</td>' +
        '<td>' + badge + '</td>' +
        '<td onclick=”event.stopPropagation()”>' +
          (st !== 'publiee'
            ? '<button class=”btn btn-l btn-sm” onclick=”event.stopPropagation();PageEnsNotes.ouvrirSaisie(\'' + evalId + '\')”>&#x270f;&#xfe0f; Saisir</button>'
            : '<button class=”btn btn-sm” style=”background:var(--g100);color:var(--g500);cursor:default”>&#x1f441; Voir</button>') +
        '</td>' +
      '</tr>';
    }).join('');
  },

  // ── Modal création évaluation ────────────────────────────────────
  ouvrirModalEval: async function() {
    const selClasse = document.getElementById('ens-ev-classe') as HTMLSelectElement | null;
    if (selClasse) {
      // Dédupliquer les classes
      const vues: Record<string, boolean> = {};
      const classes = this._classes.filter(function(c: any) {
        if (vues[c.classe_id]) return false;
        vues[c.classe_id] = true;
        return true;
      });
      selClasse.innerHTML = '<option value="">— Choisir une classe —</option>' +
        classes.map(function(c: any) {
          return '<option value="' + c.classe_id + '">' + c.classe + '</option>';
        }).join('');
      // Reset affectations
      const selAff = document.getElementById('ens-ev-affectation') as HTMLSelectElement | null;
      if (selAff) selAff.innerHTML = '<option value="">— Sélectionnez d\'abord une classe —</option>';
    }
    const selPer = document.getElementById('ens-ev-periode') as HTMLSelectElement | null;
    if (selPer) {
      selPer.innerHTML = this._periodes.map(function(p: any) {
        return '<option value="' + p.id + '">' + p.libelle + '</option>';
      }).join('');
    }
    openModal('m-ens-evaluation');
  },

  onClasseChangeEval: async function() {
    const classeEl = document.getElementById('ens-ev-classe') as HTMLSelectElement | null;
    const classeId = classeEl ? classeEl.value : '';
    const selAff = document.getElementById('ens-ev-affectation') as HTMLSelectElement | null;
    if (!selAff) return;
    if (!classeId) {
      selAff.innerHTML = '<option value="">— Sélectionnez d\'abord une classe —</option>';
      return;
    }
    // Filtrer les affectations de cet enseignant pour cette classe
    const aff = this._classes.filter(function(c: any) { return c.classe_id === classeId; });
    if (aff.length) {
      selAff.innerHTML = '<option value="">— Choisir la matière —</option>' +
        aff.map(function(a: any) {
          return '<option value="' + escapeHtml(String(a.affectation_id || '')) + '">' + escapeHtml(a.matiere || '') + '</option>';
        }).join('');
    } else {
      selAff.innerHTML = '<option value="">Aucune affectation pour cette classe</option>';
    }
  },

  creerEvaluation: async function() {
    const affId     = (document.getElementById('ens-ev-affectation') as HTMLSelectElement | null)?.value || '';
    const periodeId = (document.getElementById('ens-ev-periode')     as HTMLSelectElement | null)?.value || '';
    const type      = (document.getElementById('ens-ev-type')        as HTMLSelectElement | null)?.value || '';
    const numero    = parseInt((document.getElementById('ens-ev-numero')   as HTMLInputElement | null)?.value || '1') || 1;
    const titre     = ((document.getElementById('ens-ev-titre')      as HTMLInputElement | null)?.value || '').trim();
    const noteMax   = parseFloat((document.getElementById('ens-ev-note-max') as HTMLInputElement | null)?.value || '20') || 20;
    const date      = (document.getElementById('ens-ev-date')        as HTMLInputElement | null)?.value || '';

    if (!affId)     return toast('Sélectionnez une matière', 'w');
    if (!periodeId) return toast('Sélectionnez une période', 'w');
    if (!type)      return toast('Sélectionnez un type', 'w');

    const btn = document.getElementById('btn-ens-creer-eval') as HTMLButtonElement | null;
    if (btn) { btn.disabled = true; btn.textContent = 'Création…'; }

    try {
      const payload: Record<string, any> = { affectation_id: affId, periode_id: periodeId, type, numero, note_max: noteMax };
      if (titre) payload['titre'] = titre;
      if (date)  payload['date_evaluation'] = date;

      await Api.post('/evaluations', payload);
      closeModal('m-ens-evaluation');
      toast('Évaluation créée ✓ — vous pouvez maintenant saisir les notes', 's');
      await this.charger();
    } catch (e: any) {
      toast(e.message || 'Erreur de création', 'e');
    } finally {
      if (btn) { btn.disabled = false; btn.textContent = 'Créer l\'évaluation'; }
    }
  },

  // ── Modal saisie des notes ────────────────────────────────────────
  ouvrirSaisie: async function(evalId: string) {
    this._evalCourant = { id: evalId };

    const titreEl = document.getElementById('ens-notes-modal-titre') as HTMLElement | null;
    if (titreEl) titreEl.textContent = 'Saisie des notes';

    const tbody = document.getElementById('tb-ens-notes-saisie') as HTMLElement | null;
    if (tbody) tbody.innerHTML = '<tr><td colspan="4" style="text-align:center;padding:30px;color:var(--g400)">Chargement…</td></tr>';

    openModal('m-ens-notes');

    try {
      const evalRes = await Api.get('/evaluations/' + evalId + '/notes');
      const notesSaisies = evalRes.data || [];
      const notesMap: Record<string, any> = {};
      notesSaisies.forEach(function(n: any) { notesMap[n.utilisateur_id || n.eleve_id] = n; });

      // Si _data est vide (navigation directe depuis le dashboard), charger les évals d'abord
      if (!PageEnsNotes._data || !PageEnsNotes._data.length) {
        await PageEnsNotes.charger();
      }
      let evalInfo: any = null;
      for (let i = 0; i < (PageEnsNotes._data || []).length; i++) {
        if (PageEnsNotes._data[i].id === evalId) { evalInfo = PageEnsNotes._data[i]; break; }
      }

      PageEnsNotes._evalCourant.note_max  = evalInfo ? evalInfo.note_max  : 20;
      PageEnsNotes._evalCourant.statut    = evalInfo ? evalInfo.statut    : 'non_saisie';
      PageEnsNotes._evalCourant.classe_id = evalInfo ? evalInfo.classe_id : null;

      if (evalInfo && titreEl) {
        titreEl.textContent = (evalInfo.titre || ((evalInfo.type || '') + ' ' + (evalInfo.numero || '')).trim()) + ' — ' + (evalInfo.classe || '');
      }

      let eleves: any[] = [];
      if (evalInfo && evalInfo.classe_id) {
        const elevesRes = await Api.get('/classes/' + evalInfo.classe_id + '/eleves');
        eleves = elevesRes.data || [];
      }
      if (!eleves.length && notesSaisies.length) {
        eleves = notesSaisies.map(function(n: any) {
          return { id: n.eleve_id, nom: n.nom, prenom: n.prenom, inscription_id: n.inscription_id || '' };
        });
      }

      PageEnsNotes._renderGrille(eleves, notesMap, PageEnsNotes._evalCourant.note_max);

      const estPubliee = PageEnsNotes._evalCourant.statut === 'publiee';
      const btnPublier = document.getElementById('btn-ens-publier-notes') as HTMLElement | null;
      const btnSauver  = document.getElementById('btn-ens-sauver-notes')  as HTMLElement | null;
      if (btnPublier) btnPublier.style.display = estPubliee ? 'none' : '';
      if (btnSauver)  btnSauver.style.display  = estPubliee ? 'none' : '';

    } catch (e) {
      if (tbody) tbody.innerHTML = '<tr><td colspan="4" style="text-align:center;padding:30px;color:var(--rouge)">Erreur : ' + escapeHtml(e.message || '') + '</td></tr>';
    }
  },

  _renderGrille: function(eleves: any[], notesMap: Record<string, any>, noteMax: number) {
    const tbody = document.getElementById('tb-ens-notes-saisie') as HTMLElement | null;
    if (!tbody) return;
    if (!eleves.length) {
      tbody.innerHTML = '<tr><td colspan="4" style="text-align:center;padding:30px;color:var(--g400)">Aucun élève</td></tr>';
      return;
    }
    tbody.innerHTML = eleves.map(function(el: any, i: number) {
      const note = notesMap[el.id] || {};
      const absent = note.est_absent || false;
      return '<tr id="nr-' + i + '">' +
        '<td><span style="font-weight:600">' + escapeHtml(el.nom || '') + '</span> ' + escapeHtml(el.prenom || '') +
          '<input type="hidden" class="nr-eleve-id" value="' + escapeHtml(String(el.id || '')) + '">' +
          '<input type="hidden" class="nr-inscription" value="' + escapeHtml(String(el.inscription_id || '')) + '">' +
        '</td>' +
        '<td style="text-align:center"><input type="checkbox" class="nr-absent" ' + (absent ? 'checked' : '') +
          ' onchange="PageEnsNotes._toggleAbsent(this,' + i + ')"></td>' +
        '<td><input type="number" class="fi nr-valeur" min="0" max="' + noteMax + '" step="0.5"' +
          ' value="' + (note.valeur != null ? note.valeur : '') + '"' +
          ' placeholder="/' + noteMax + '" ' + (absent ? 'disabled' : '') +
          ' style="width:90px;padding:4px 8px;font-size:13px"></td>' +
        '<td><input type="text" class="fi nr-appreciation" placeholder="Appréciation…"' +
          ' value="' + escapeHtml(note.appreciation || '') + '"' +
          ' style="font-size:12px;padding:4px 8px"></td>' +
      '</tr>';
    }).join('');
  },

  _toggleAbsent: function(checkbox: HTMLInputElement, i: number) {
    const row = document.getElementById('nr-' + i);
    if (!row) return;
    const input = row.querySelector('.nr-valeur') as HTMLInputElement | null;
    if (input) { input.disabled = checkbox.checked; if (checkbox.checked) input.value = ''; }
  },

  sauvegarderNotes: async function() {
    const ev = this._evalCourant;
    if (!ev) return;

    const rows = document.querySelectorAll('#tb-ens-notes-saisie tr[id^="nr-"]');
    const notes: Record<string, any>[] = [];
    rows.forEach(function(row: Element) {
      const eleveIdEl       = row.querySelector('.nr-eleve-id')     as HTMLInputElement | null;
      const inscriptionIdEl = row.querySelector('.nr-inscription')  as HTMLInputElement | null;
      const absentEl        = row.querySelector('.nr-absent')       as HTMLInputElement | null;
      const valeurEl        = row.querySelector('.nr-valeur')       as HTMLInputElement | null;
      const appreciationEl  = row.querySelector('.nr-appreciation') as HTMLInputElement | null;

      const eleveId       = eleveIdEl       ? eleveIdEl.value       : '';
      const inscriptionId = inscriptionIdEl ? inscriptionIdEl.value : '';
      const absent        = absentEl        ? absentEl.checked      : false;
      const valeurRaw     = valeurEl        ? valeurEl.value        : '';
      const appreciation  = (appreciationEl && appreciationEl.value.trim()) ? appreciationEl.value.trim() : undefined;
      const valeur        = valeurRaw !== '' ? parseFloat(valeurRaw) : null;

      if (eleveId) notes.push({ eleve_id: eleveId, inscription_id: inscriptionId, est_absent: absent, absence_justifiee: false, valeur, appreciation });
    });

    if (!notes.length) return toast('Aucune note à enregistrer', 'w');

    const btn = document.getElementById('btn-ens-sauver-notes') as HTMLButtonElement | null;
    if (btn) { btn.disabled = true; btn.textContent = 'Enregistrement…'; }

    try {
      await Api.put('/evaluations/' + ev.id + '/notes', { notes });
      toast(notes.length + ' notes enregistrées ✓', 's');
      await PageEnsNotes.charger();
    } catch (e: any) {
      toast(e.message || 'Erreur d\'enregistrement', 'e');
      throw e;
    } finally {
      if (btn) { btn.disabled = false; btn.textContent = 'Enregistrer'; }
    }
  },

  publierNotes: async function() {
    var ev = this._evalCourant;
    if (!ev) return;
    if (!confirm('Publier les notes ? Les parents seront notifiés par SMS/WhatsApp.')) return;

    const btn = document.getElementById('btn-ens-publier-notes') as HTMLButtonElement | null;
    if (btn) { btn.disabled = true; btn.textContent = 'Publication…'; }

    try {
      await PageEnsNotes.sauvegarderNotes();
      await Api.put('/evaluations/' + ev.id + '/publier', {});
      toast('Notes publiées — parents notifiés ������', 's');
      closeModal('m-ens-notes');
      await PageEnsNotes.charger();
    } catch (e: any) {
      toast(e.message || 'Erreur de publication', 'e');
    } finally {
      if (btn) { btn.disabled = false; btn.textContent = '������ Publier (notif parents)'; }
    }
  },
};

(window as any).PageEnsNotes = PageEnsNotes;
PAGE_HOOKS['ens-notes'] = () => PageEnsNotes.init();
